...
class Detector:
    ...
    def detect_players_image(
        self, img_src, display_detection=True, save_detection=False, ret_img=False
    ):
        ...
        if detections is not None and len(detections) < 4:
            ...
            for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
                if classes[int(cls_pred)] in object_names:
                    ...
                    cv2.rectangle(
                        out_img, (x1, y1), (x1 + box_w, y1 + box_h), (128, 0, 128), 2
                    )  # purple bbox
        ...
        if save_detection == True:
            if type(img_src) == str:
                print(
                    "Output image can be found here: "
    ...
    def detect_players_video(self, video_path):
        ...
        out = cv2.VideoWriter(
            video_path.replace(".mp4", "-out.mp4"), fourcc, fps, (w, h)
        )
        for i in range(len(out_video)):
            out.write(out_video[i])
        print(
            "Output Video can be found here: " + video_path.replace(".mp4", "-out.mp4")
        )
        out.release()
            ...
            if len(centerbottom) != 0:
                for i in range(0, len(centerbottom), 2):
                    ...
                    bbox = patches.Rectangle(
                        (pointsOut1[0][0][0], pointsOut1[0][0][1]),
                        3,
                        3,
                        linewidth=2,
                        edgecolor="r",
