...
class Detector:
    ...
    def detect_image(self, model, img):
        ...
        self.img = img
           ...
           # scale and pad image
        ratio = min(self.img_size /
                    self.img.size[0], self.img_size/self.img.size[1])
    ...
    def detect_players_image(self, img_src, display_detection=True, save_detection=False, ret_img=False):
        ...
        if detections is not None and len(detections) < 4:
            ...
            for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
                if classes[int(cls_pred)] in object_names:
                    ...
                    color = tuple(
                        [255*x for x in bbox_colors[int(np.where(unique_labels == int(cls_pred))[0])]])
                    cv2.putText(img=out_img, text=label, org=(
                        x1, y1 - 10), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(255, 255, 255), thickness=2)
                    cv2.rectangle(out_img, (x1, y1), (x1 + box_w,
                                  y1 + box_h), (128, 0, 128), 2)  # purple bbox
        ...
        if save_detection == True:
            if type(img_src) == str:
                ...
                cv2.imwrite(os.getcwd()+"/output.jpg",
                            cv2.cvtColor(out_img, cv2.COLOR_RGB2BGR))
    ...
    def detect_players_video(self, video_path):
        ...
        print("Output Video can be found here: " +
              video_path.replace(".mp4", "-out.mp4"))
        out.release()
